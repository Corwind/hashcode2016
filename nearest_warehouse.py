...
def next_warehouses(drone, order, warehouses):
  useful_warehouses = []
  distances = []
  for w in warehouses:
  ...
  for ware in useful_warehouses:
    distances.append(get_distance(ware, warehouses[0]))
  return useful_warehouses[distances.index(min(distances))]

if __name__ == '__main__':
  infos = parse(sys.argv[1])
  a = next_warehouses(infos.drones[0], infos.orders[0], infos.warehouses)
